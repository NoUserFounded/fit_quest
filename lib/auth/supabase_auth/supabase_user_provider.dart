...
class FitQuestSupabaseUser extends BaseAuthUser {
  FitQuestSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fitQuestSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FitQuestSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
